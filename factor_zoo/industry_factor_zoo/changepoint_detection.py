...
def single_step_changepoint(window_end, time_series_data, lookback_window_length, use_kM_hyp_to_initialise_kC):
    ...
    try:
        ...
        cp_score, cp_loc, cp_loc_normalised = np.nan, np.nan, np.nan

    return {
        "date": window_date,
        "t": time_index,
        "cp_location": cp_loc,
        "cp_location_norm": cp_loc_normalised,
        "cp_score": cp_score
    }


def get_single_asset_changepoint_detection(
        time_series_data: pd.DataFrame,
    ...
    """
    code = time_series_data.index.get_level_values(1)[0]
    time_series_data = time_series_data.droplevel(1)
    ...
    time_series_data = time_series_data[["date", "change_pct"]]
    time_series_data["X"] = time_series_data.index.astype(float)
    time_series_data = time_series_data.rename(columns={"change_pct": "Y"})

    # f = partial(single_step_changepoint,
    #             time_series_data=time_series_data, lookback_window_length=lookback_window_length,
    #             use_kM_hyp_to_initialise_kC=use_kM_hyp_to_initialise_kC)
    # l = [window_end for window_end in range(lookback_window_length + 1, len(time_series_data))]
    #
    # with ProcessPoolExecutor(2) as executor:
    #     res = list(tqdm.tqdm(executor.map(f, l, chunksize=1), total=len(l)))

    for window_end in range(lookback_window_length + 1, len(time_series_data)):
        r = single_step_changepoint(window_end, time_series_data, lookback_window_length, use_kM_hyp_to_initialise_kC)
        res.append(r)
