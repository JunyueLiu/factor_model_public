...
def MAMA(inputs, fastlimit: float = 0.5, slowlimit: float = 0.05, price_type: str = 'close'):
    """
    {'name': 'MAMA',
    'group': 'Overlap Studies',
...
def SAR(inputs, acceleration: float = 0.02, maximum: float = 0.2, prices=None):
    """
    {'name': 'SAR',
     'group': 'Overlap Studies',
    ...
    if not utils.check(inputs, prices):
        raise ValueError('')
    return indicator(inputs, acceleration=acceleration, maximum=maximum, prices=prices)
...
def T3(inputs, period: int = 5, vfactor: float = 0.7, price_type: str = 'close'):
    """
    {'name': 'T3',
     'group': 'Overlap Studies',
